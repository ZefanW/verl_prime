...
class RewardManager():
    ...
    def __init__(self, tokenizer, num_examine,config) -> None:
        ...
        self.config=config
        verifier_name = self.config.verifier.get('name','default')
    ...
    def verify(self, data):
        ...
        reward_metrics = {}
        for ability in list(set(data.batch['ability'])):
...
def main_task(config):
    ...
    reward_fn = RewardManager(tokenizer=tokenizer, num_examine=0, config=config.verifier) # note: verifier is called both inside reward_fn and outside.

    # Note that we always use function-based RM for validation
    val_reward_fn = RewardManager(tokenizer=tokenizer, num_examine=1,config=config.verifier)
