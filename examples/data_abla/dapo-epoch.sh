...
PROJECT_NAME='epoch-exp'
EXPERIMENT_NAME="rloo-${DATASET_BASE}-n${N}-epoch${E}-range${RANGE}-choosebyent"
...
python3 -m recipe.prime.main_prime \
    ...
    actor_rollout_ref.actor.entropy_coeff=[0.2,3e-5,1e-1] \
    actor_rollout_ref.actor.entropy_type=Adaptive \
    actor_rollout_ref.actor.ppo_epochs=${RANGE} \
    actor_rollout_ref.actor.ppo_epochs_max=${E} \
    actor_rollout_ref.actor.shuffle=True \
#    actor_rollout_ref.actor.token_filter_type=entropy \
#    actor_rollout_ref.actor.token_filter_percentage=0.2 \
