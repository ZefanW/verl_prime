...
my_array=(aime amc math500 qwen)
#my_array=(leetcode livecodebench)

cd /home/test/test05/whb/project/eval-cgq-new
...
if [[ " ${my_array[@]} " =~ " gpqa " ]]; then
    ...
    python3 -u scripts/eval/gpqa/run_gpqa_chat.py \
        --model $MODEL_CKPT \
...
if [[ " ${my_array[@]} " =~ " mmlu " ]]; then
    ...
    python3 -u scripts/eval/mmlu/evaluate_mmlu_quicktest.py \
        --model $MODEL_CKPT \
